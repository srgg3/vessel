...
ofstream out("output_pyramida.txt");
...
void heapify(vector<people>& a, int n, int i){
    int k = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;

    if (left < n and a[left].salary > a[k].salary){
        k = left;
    }

    if (right < n and a[right].salary > a[k].salary){
        k = right;
    }
